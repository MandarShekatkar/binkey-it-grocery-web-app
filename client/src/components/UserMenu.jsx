...
const UserMenu = ({close}) => {
  ...
  return (
    <div>
      ...
      <span className='max-w-53 text-ellipsis line-clamp-1'>{user.name || user.mobile}<span className='text-medium text-red-600' >{user.role === "ADMIN" ? "(Admin)" : ""}</span></span>
        <Link onClick={handleClose} to={"/dashboard/profile"} className='hover:text-primary-200'>
      <FiExternalLink size={15}/></Link>
      </div>
      ...
      {
        isAdmin(user.role) && (
          <Link onClick={handleClose} to={"/dashboard/subcategory"}className='px-2 
...
<Link onClick={handleClose} to={"/dashboard/product"}className='px-2 
        ...
        hover:bg-orange-200'>Save Address</Link>
